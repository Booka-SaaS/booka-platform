...
export function buildClientesRouter() {
  ...
  router.get('/:id', async (request, response, next) => {
    try {
      const authenticatedRequest = request as AuthenticatedRequest;
      const result = await getCliente(authenticatedRequest.auth!.userId, request.params.id);
      response.json(result);
    } catch (error) {
      next(error);
    }
  ...
  router.put('/:id', async (request, response, next) => {
    try {
      ...
      const result = await updateCliente(authenticatedRequest.auth!.userId, request.params.id, payload);
      response.json(result);
    } catch (error) {
      next(error);
    }
  ...
  router.delete('/:id', async (request, response, next) => {
    try {
      const authenticatedRequest = request as AuthenticatedRequest;
      const result = await deleteCliente(authenticatedRequest.auth!.userId, request.params.id);
      response.json(result);
    } catch (error) {
      next(error);
    }
